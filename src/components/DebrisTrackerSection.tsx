...
function Earth() {
  ...
  return (
    <mesh ref={meshRef}>
      ...
      <meshStandardMaterial color="#1a3a5c" emissive="#0a1a2c" emissiveIntensity={0.3} />
      {/* Atmosphere glow */}
      <mesh>
      ...
      <mesh>
        ...
        <meshStandardMaterial wireframe color="#336699" transparent opacity={0.2} />
      </mesh>
    </mesh>
  );
}
...
function DebrisCloud({ positions }: { positions: Float32Array }) {
  ...
  return (
    <points ref={ref}>
      ...
      <pointsMaterial size={0.02} color="#ff6f61" transparent opacity={0.8} sizeAttenuation />
    </points>
  );
}
  ...
  return (
    <div className="w-full h-[400px] md:h-[500px]">
      <Canvas camera={{ position: [0, 2, 4], fov: 45 }}>
        ...
        <pointLight position={[-3, 2, -3]} intensity={0.4} color="#ff6f61" />
        <Earth />
        {debrisPositions.length > 0 && <DebrisCloud positions={debrisPositions} />}
        <OrbitRing radius={1.5} color="#ff6f61" />
        <OrbitRing radius={1.8} color="#ff9e7a" opacity={0.1} />
...
// Scale real orbital position to 3D scene
function scalePosition(eciPos: { x: number; y: number; z: number }): [number, number, number] {
  const EARTH_RADIUS_KM = 6371;
  const SCALE = 1 / EARTH_RADIUS_KM; // 1 unit = Earth radius
...
const DebrisTrackerSection = () => {
  ...
  const fetchDebrisData = useCallback(async () => {
    try {
      ...
      // Fallback: generate mock debris positions
      const points: DebrisPoint[] = [];
      for (let i = 0; i < 150; i++) {
      ...
      <div className="section-container">
        ...
        >
          ...
          <h2 className="text-3xl md:text-4xl font-display font-bold mb-4">
            Real-Time Space Debris Tracker
          </h2>
          <p className="text-muted-foreground max-w-xl mx-auto text-sm">
            Tracking {debrisCount > 0 ? debrisCount.toLocaleString() : "..."} debris objects from
            Celestrak TLE data with SGP4 orbital propagation, plus Near-Earth Objects from NASA NeoWs.
          </p>
        </motion.div>
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          className="glass-card p-2 mb-8 overflow-hidden"
        >
          ...
          <div className="flex items-center justify-center gap-4 p-3 text-xs text-muted-foreground">
            <span className="flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-primary" /> Debris (Cosmos-1408)
            </span>
            <span className="flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-accent" /> Orbit rings
            </span>
            <span className="flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-[hsl(210,60%,50%)]" /> Earth
            </span>
          </div>
        </motion.div>

        {/* Tabs: Debris List / NEO Table */}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                ...
                <tbody>
                  {neoObjects.map((neo, i) => (
                    <tr key={i} className="border-b border-border/50 hover:bg-secondary/30 transition-colors">
                      ...
                      <td className="p-3 text-right text-muted-foreground font-mono">
                        {neo.estimated_diameter_min.toFixed(0)}–{neo.estimated_diameter_max.toFixed(0)}
                      </td>
                  ...
                  {neoObjects.length === 0 && (
                    <tr>
                      <td colSpan={5} className="p-6 text-center text-muted-foreground">No NEO data available</td>
                    </tr>
                  )}
                </tbody>
              </table>
